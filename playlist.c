...
void shuffle() {
  ...
  while (j--) {
    ...
    n = remove_song(n, rand->name, rand->artist);
  }
  printf("\n");
}
...
void delete_song( char n[256], char a[256] ) {
  printf("\ndelete %s \n", n);
  table[a[0]-97] = remove_song(table[a[0]-97], n, a);
  printf( "New libary: \n" );
  print_lib();
}
